 ...
 public:
  ...
  gxf_result_t stop() override {
    if (counter_ != expected_counter_) {
      GXF_LOG_ERROR("counter does not match: %ld vs %ld", counter_, expected_counter_.get());
      return GXF_FAILURE;
    }
    return GXF_SUCCESS;
  }
 ...
 public:
  gxf_result_t tick() override {
    ...
    GXF_ASSERT(zoo_tensor->bytes_size() == 2 * 3 * 4 * 8 /* sizeof(double) */,
               "Bad zoo bytes_size %lu", zoo_tensor->bytes_size());
...
class TestConfigurationSet: public Codelet {
 public:
  ...
  gxf_result_t tick() override {
    ...
    GXF_LOG_INFO("uint64_value: %lu", uint64_value_.get());
