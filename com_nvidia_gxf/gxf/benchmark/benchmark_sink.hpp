...
class BenchmarkSink : public BenchmarkSinkBase {
 public:
 ...
 private:
  ...
  gxf::Parameter<gxf::Handle<BenchmarkPublisher>> benchmark_publisher_;
  gxf::Parameter<std::vector<gxf::Handle<PerformanceCalculatorBase>>>
      performance_calculators_;
  gxf::Parameter<bool> use_received_acqtime_as_published_;

  bool is_benchmarking_{false};

  // Message arrival timestamps
  std::vector<std::chrono::nanoseconds> received_timestamps_;

  // A list of acqtime obtained from received messages
  std::vector<std::chrono::nanoseconds> published_acqtimes_;
};
