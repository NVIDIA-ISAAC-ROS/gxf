...
class HostToDeviceApp : public Application {
 public:
  void compose() override {
    ...
    // create an entity to copy messages from host to device
    auto copier = makeEntity<TensorCopier>("Copier", Arg("allocator") =
                      makeResource<UnboundedAllocator>("allocator"), Arg("mode", 0));
    ...
    comparator->add<test::StepCount>("Count_host", Arg("expected_count", 10));

    // add data flow connection tx -> rx
    connect(generator_host, copier);
    connect(copier, comparator, PortPair{"transmitter", "actual"});
    connect(generator_device, comparator, PortPair{"output", "expected"});
...
TEST(TestApp, HostToDeviceApp) {
  auto app = create_app<HostToDeviceApp>();
