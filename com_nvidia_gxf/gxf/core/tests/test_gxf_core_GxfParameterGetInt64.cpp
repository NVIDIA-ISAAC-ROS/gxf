...
class GxfParameterGetInt64_Test : public ::testing::Test {
 public:
  void SetUp() {
...
TEST_F(GxfParameterGetInt64_Test, ValidParameter) {
...
TEST_F(GxfParameterGetInt64_Test, InvalidContext) {
  GXF_ASSERT_EQ(GxfParameterGetInt64(kNullContext, cid, "int64_var", &value),
                GXF_CONTEXT_INVALID);
}

TEST_F(GxfParameterGetInt64_Test, InvalidUid) {
  GXF_ASSERT_EQ(GxfParameterGetInt64(context, kNullUid, "int64_var", &value),
                GXF_PARAMETER_NOT_FOUND);
}

TEST_F(GxfParameterGetInt64_Test, InvalidKey) {
  int64_t int64_var = 101;
  GXF_ASSERT_SUCCESS(GxfParameterSetInt64(context, cid, "int64_var", int64_var));
  GXF_ASSERT_EQ(GxfParameterGetInt64(context, cid, "int64_invalid_var", &value),
                GXF_PARAMETER_NOT_FOUND);
}

TEST_F(GxfParameterGetInt64_Test, InvalidParameterType) {
  double float64_var = 101.1;
  GXF_ASSERT_SUCCESS(GxfParameterSetFloat64(context, cid, "float64_var", float64_var));
  GXF_ASSERT_EQ(GxfParameterGetInt64(context, cid, "float64_var", &value),
