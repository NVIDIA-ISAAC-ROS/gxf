...
TEST_F(GxfGraphDeactivateTest, InvalidContext) {
  GXF_ASSERT_EQ(GxfGraphDeactivate(nullptr), GXF_CONTEXT_INVALID);
}
...
TEST_F(GxfGraphDeactivateTest, InvalidDeactivateAfterRunAsync) {
  gxf_uid_t eid = kNullUid;
  const GxfEntityCreateInfo entity_create_info = {"abc", 1};
  ASSERT_EQ(GxfCreateEntity(context, &entity_create_info, &eid), GXF_SUCCESS);
  GXF_ASSERT_NE(eid, kNullUid);
  ...
  ASSERT_DEATH(GxfGraphDeactivate(context), ".*");
}
