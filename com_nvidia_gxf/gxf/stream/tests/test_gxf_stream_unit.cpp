...
TEST(StreamSync, SetInvalidCudaStream) {
  gxf_context_t context;
  ASSERT_EQ(GxfContextCreate(&context), GXF_SUCCESS);
  ...
  Stream* streamSync = static_cast<Stream*>(pointer);
  ASSERT_EQ(streamSync->setCudaStream(static_cast<SyncType>(0), nullptr), GXF_ARGUMENT_INVALID);
}

TEST(StreamSync, SetInvalidSyncObject) {
  constexpr int32_t signaler = static_cast<int32_t>(SyncType::GXF_STREAM_SIGNALER_CUDA);
  constexpr int32_t waiter = static_cast<int32_t>(SyncType::GXF_STREAM_WAITER_CUDA);
  constexpr int32_t signaler_gpu_id = 0;
  constexpr int32_t waiter_gpu_id = 0;
  ...
  void *syncObj{nullptr};
  ASSERT_EQ(
      streamSync->allocate_sync_object(static_cast<SyncType>(5), static_cast<SyncType>(waiter),
                                       reinterpret_cast<void**>(&syncObj)),
      GXF_ARGUMENT_INVALID);
  ASSERT_EQ(
      streamSync->allocate_sync_object(static_cast<SyncType>(signaler), static_cast<SyncType>(15),
                                       reinterpret_cast<void**>(&syncObj)),
      GXF_ARGUMENT_INVALID);
}

TEST(StreamSync, InvalidArgStreamSyncSignalAndWait) {
  constexpr int32_t signaler = static_cast<int32_t>(SyncType::GXF_STREAM_SIGNALER_CUDA);
  constexpr int32_t waiter = static_cast<int32_t>(SyncType::GXF_STREAM_WAITER_CUDA);
  constexpr int32_t signaler_gpu_id = 0;
  constexpr int32_t waiter_gpu_id = 0;
  ...
  ASSERT_EQ(streamSync->signalSemaphore(), GXF_ARGUMENT_INVALID);
  ASSERT_EQ(streamSync->waitSemaphore(), GXF_ARGUMENT_INVALID);
